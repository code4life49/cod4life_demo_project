...
public class MeasureAdjustSTLRenderer extends BaseRenderer {
    ...
    public MeasureAdjustSTLRenderer() {
        setTransLation_Z();
        setBTransLation_Z();
        setCTransLation_Z();
    }
...
//        gl.glEnable(GL10.GL_BLEND);
        ...
        if (MeasureAdjustStlUtils.getStlModel() != null) {
            gl.glPushMatrix();
            gl.glColor4f(132 / 256f, 66 / 256f, 66 / 256f, 1.0f);
            gl.glRotatef(180f, 0, 0, 1);
            gl.glRotatef(180f, 1, 0, 0);

            gl.glRotatef(-forwardAngle, (float) Math.cos(abductionAngle * Math.PI / 180), (float) Math.sin(abductionAngle * Math.PI / 180), 0);
            gl.glRotatef(abductionAngle, 0, 0, 1);
            MeasureAdjustStlUtils.getStlModel().draw(gl);
            gl.glPopMatrix();
        }

        gl.glColor4f(220 / 256f, 223 / 256f, 227 / 256f, alphaValue);
        if (MeasureAdjustStlUtils.getcStlModel() != null) {
            gl.glPushMatrix();
            gl.glRotatef(180f, 0, 0, 1);
            gl.glRotatef(180f, 1, 0, 0);
//            gl.glRotatef(rollAngleAdjust, 1, 0, 0);
//            gl.glRotatef(pitchAngleAdjust, 0, 1, 0);
//            gl.glRotatef( azimuthAngleAdjust, 0, 0, 1);

//            gl.glRotatef(forwardAngleAdjust, 0, 1,(float) (1-Math.tan(abductionAngleAdjust/180*Math.PI)));
//            gl.glRotatef(abductionAngleAdjust, 1, 0, 0);


            gl.glRotatef(-forwardAngleAdjust, (float) Math.cos(abductionAngleAdjust * Math.PI / 180), (float) Math.sin(abductionAngleAdjust * Math.PI / 180), 0);
            gl.glRotatef(abductionAngleAdjust, 0, 0, 1);

            MeasureAdjustStlUtils.getcStlModel().draw(gl);
            gl.glPopMatrix();
        }


        gl.glColor4f(red, green, blue, 1.0f);
        if (MeasureAdjustStlUtils.getbStlModel() != null) {
            gl.glPushMatrix();
            gl.glRotatef(180f, 0, 0, 1);
            gl.glRotatef(180f, 1, 0, 0);
            MeasureAdjustStlUtils.getbStlModel().draw(gl);
            gl.glPopMatrix();
        }
...
//		 gl.glBlendFunc(GL10.GL_ONE, GL10.GL_SRC_COLOR);
    ...
    private void setTransLation_Z() {
        //算x、y轴差值
        float distance_x = MeasureAdjustStlUtils.getStlModel().maxX - MeasureAdjustStlUtils.getStlModel().minX;
        float distance_y = MeasureAdjustStlUtils.getStlModel().maxY - MeasureAdjustStlUtils.getStlModel().minY;
        float distance_z = MeasureAdjustStlUtils.getStlModel().maxZ - MeasureAdjustStlUtils.getStlModel().minZ;
    ...
    private void setBTransLation_Z() {
        //算x、y轴差值
        float distance_x = MeasureAdjustStlUtils.getbStlModel().maxX - MeasureAdjustStlUtils.getbStlModel().minX;
        float distance_y = MeasureAdjustStlUtils.getbStlModel().maxY - MeasureAdjustStlUtils.getbStlModel().minY;
        float distance_z = MeasureAdjustStlUtils.getbStlModel().maxZ - MeasureAdjustStlUtils.getbStlModel().minZ;
    ...
    private void setCTransLation_Z() {
        if (MeasureAdjustStlUtils.getcStlModel() == null) return;
        //算x、y轴差值
        float distance_x = MeasureAdjustStlUtils.getcStlModel().maxX - MeasureAdjustStlUtils.getcStlModel().minX;
        float distance_y = MeasureAdjustStlUtils.getcStlModel().maxY - MeasureAdjustStlUtils.getcStlModel().minY;
        float distance_z = MeasureAdjustStlUtils.getcStlModel().maxZ - MeasureAdjustStlUtils.getcStlModel().minZ;
